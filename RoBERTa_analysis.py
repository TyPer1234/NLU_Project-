...
# Load the Facebook RoBERTa model for hate speech detection
def load_hate_speech_model():
    """Load Facebook's RoBERTa model for hate speech detection."""
    print("Loading Facebook RoBERTa hate speech model...")
    ...
    try:
        ...
        print(f"Failed to load Facebook RoBERTa model: {e}")
        raise RuntimeError("Could not load Facebook RoBERTa model for hate speech analysis")
...
def aggregate_hate_speech_by_year_party(df):
    ...
    # Convert label to binary (1 for hate speech, 0 for non-hate speech)
    # Updated: Assume that 'LABEL_0' corresponds to hate speech.
    filtered_df['is_hate_speech'] = filtered_df['label'].apply(lambda x: 1 if x == 'LABEL_0' else 0)
...
def main(statements_csv_path, folder_name=None, output_base_dir='./output'):
    ...
    if folder_name is None:
        ...
        folder_name = f"{input_file}_FacebookRoBERTa_{timestamp}"
    ...
    # Attempt to aggregate by year and party if those columns exist
    if 'year' in df.columns and 'party' in df.columns:
        ...
        print("\nSkipping aggregation - 'year' or 'party' columns not found in the data")
...
if __name__ == "__main__":
    ...
    # main("debate_statements_with_party.csv", folder_name="FacebookRoBERTa_Analysis")
    pass